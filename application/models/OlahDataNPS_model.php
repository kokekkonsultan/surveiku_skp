...
class OlahDataNPS_model extends CI_Model
{
    ...
    private function _get_datatables_query($table_identity)
    {
        ...
        $this->db->from("(SELECT *, (SELECT SUM(skor_jawaban) FROM jawaban_pertanyaan_nps_$table_identity WHERE jawaban_pertanyaan_nps_$table_identity.id_responden = responden_$table_identity.id) AS total_skor FROM responden_$table_identity) AS rspdn_$table_identity");
