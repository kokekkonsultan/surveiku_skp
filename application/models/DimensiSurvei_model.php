...
class DimensiSurvei_model extends CI_Model
{
    ...
    {
        $this->db->select("dimensi_$table_identity.*, aspek_$table_identity.nama_aspek");
        $this->db->from('dimensi_' . $table_identity);
        $this->db->join("aspek_$table_identity", "aspek_$table_identity.id = dimensi_$table_identity.id_aspek", "left");
