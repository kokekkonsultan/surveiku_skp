...
<form
    action="{{base_url() . $ci->session->userdata('username') . '/' . $ci->uri->segment(2) . '/pertanyaan-terbuka/update-detail-alur'}}"
    ...
    <table class="table table-bordered table-hover">
        ...
        @foreach($kategori_terbuka->result() as $row)
        <tr>
            ...
            <td>
                <select class="form-control form-control-sm" name="is_next_step[]">
                    ...
                    } elseif($object->is_letak_pertanyaan == 3) {
                    $pertanyaan_terbuka = $ci->db->query("SELECT * FROM
                    pertanyaan_terbuka_$manage_survey->table_identity WHERE is_letak_pertanyaan = 3 &&
                    SUBSTR(nomor_pertanyaan_terbuka,2) > $nomor_pertanyaan_terbuka");
                    ...
                    <option value="{{$value->nomor_pertanyaan_terbuka}}"
                        <?php echo $row->is_next_step == $value->nomor_pertanyaan_terbuka ? 'selected' : '' ?>>Lanjutkan
                        Ke {{$value->nomor_pertanyaan_terbuka}}</option>
                    @endforeach
