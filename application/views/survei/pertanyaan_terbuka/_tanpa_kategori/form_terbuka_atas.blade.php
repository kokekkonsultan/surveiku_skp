...
<div class="mt-10 mb-10" id="display_{{$row_terbuka_atas->nomor_pertanyaan_terbuka}}">
    ...
    <table class="table table-borderless" width="100%" border="0">
        <tr>
            <td width="5%" valign="top">{!! $row_terbuka_atas->nomor_pertanyaan_terbuka . '' . $is_required_ta !!}.
            </td>
            ...
            <td style="font-weight:bold;" width="95%">
                ...
                <div class="{{$model_ta}}-inline mb-2">
                    <label class="{{$model_ta}} {{$model_ta}}-outline {{$model_ta}}-success {{$model_ta}}-lg" style="font-size: 16px;">
                        <input type="{{$model_ta}}" name="jawaban_pertanyaan_terbuka[{{ $row_terbuka_atas->id_pertanyaan_terbuka }}][]" value="{{ $value_terbuka->pertanyaan_ganda }}" class="terbuka_{{ $row_terbuka_atas->nomor_pertanyaan_terbuka }}" <?= $value_terbuka->pertanyaan_ganda == $row_terbuka_atas->jawaban ? 'checked' : '' ?> {{ $is_required_ta_i }} <?= in_array($value_terbuka->pertanyaan_ganda, unserialize($row_terbuka_atas->jawaban)) ? 'checked' : ''; ?>>
                        <span></span>
                        {{ $value_terbuka->pertanyaan_ganda}}
                    </label>
                </div>
                @endforeach
                ...
                <input class="form-control" name="jawaban_lainnya[{{ $row_terbuka_atas->id_pertanyaan_terbuka }}]" value="{{$row_terbuka_atas->jawaban_lainnya}}" pattern="^[a-zA-Z0-9.,\s]*$|^\w$" placeholder="Masukkan jawaban lainnya ..." id="terbuka_lainnya_{{ $row_terbuka_atas->nomor_pertanyaan_terbuka }}" <?= in_array('Lainnya', unserialize($row_terbuka_atas->jawaban)) ? 'required' : 'style="display:none"'; ?>>

                <small id="text_terbuka_{{ $row_terbuka_atas->nomor_pertanyaan_terbuka }}" class="text-danger" <?= in_array('Lainnya', unserialize($row_terbuka_atas->jawaban)) ? '' : 'style="display:none"'; ?>>**Pengisian
        ...
        <tr>
            ...
            <td style="font-weight:bold;" width="95%">
                ...
                <textarea class="form-control terbuka_{{ $row_terbuka_atas->nomor_pertanyaan_terbuka }}" type="text" name="jawaban_pertanyaan_terbuka[{{ $row_terbuka_atas->id_pertanyaan_terbuka }}][]" placeholder="Masukkan Jawaban Anda ..." <?= $row_terbuka_atas->stts_required ?>>{{ $row_terbuka_atas->jawaban != '' ?  implode("", unserialize($row_terbuka_atas->jawaban)) : ''; }}</textarea>
            </td>
        </tr>
